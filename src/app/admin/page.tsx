  ...
  return (
    <div className="mx-auto max-w-md space-y-5">
      ...
      <form action={createQuestion} className="space-y-4">
        ...
        {/* Inline new-game fields. Hidden unless gameId === "new" via CSS sibling
            selector trick isn't reliable across browsers in forms, so we just
            always render them; they're ignored unless gameId === "new". */}
        ...
        <button
          type="submit"
          className="w-full rounded-lg bg-[var(--primary)] px-4 py-3 text-sm font-semibold text-[var(--bg)] transition-colors hover:bg-[var(--primary-hi)] hover:ring-2 hover:ring-white/40"
        >
          Post question
        </button>
      </form>

      <p className="pt-4 text-center text-[10px] uppercase tracking-[0.18em] text-[var(--text-muted)]">
        Settle &amp; void controls live on each contest page (admin-only buttons).
      </p>
    </div>
  );
}
